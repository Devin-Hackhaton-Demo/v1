...
const HELP = {
  preview: ['About Coffeenator', 'Themes, profiles, text imports and manual subscription tracking work locally on this device. Chat replies are generated through the server-side Anthropic proxy. On the Connectors page you add your own API keys; they stay in this browser and are only sent to the server to run a check. Cloud sync is not available.'],
  ...
  composio: ['Your API keys', 'Each key is kept only in this browser (localStorage). When you press Connect or Check, it is sent once to this app’s server, which calls the provider and returns only whether the key works. Keys are never stored or logged on the server. Healthy means the provider accepted the key at the time shown. Gmail, Google Calendar and Google Drive connect through your Composio project.'],
...
const KEY_STATE = {
  healthy: ['Healthy', 'success', 'check'],
  unauthorized: ['Key rejected', 'danger'],
  unreachable: ['Not reachable', 'warning'],
  unchecked: ['Not checked', 'neutral'],
};
const timeLabel = (value) => value && Number.isFinite(Date.parse(value)) ? new Intl.DateTimeFormat('en', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }).format(new Date(value)) : '';
const googleCount = (connectorId) => keys.composio?.status === 'healthy' ? keys.composio.toolkits?.[GOOGLE_TOOLKITS[connectorId]] || 0 : 0;

// Status pill for a catalog card, or '' when nothing is connected.
function connectorPill(connector) {
  const provider = CONNECTOR_KEYS[connector.id];
  if (provider === 'google') return googleCount(connector.id) ? statePill('Active in Composio', 'success', 'check') : '';
  return keys[provider] ? statePill(...KEY_STATE[keys[provider].status]) : '';
}
const isConnected = (connector) => CONNECTOR_KEYS[connector.id] === 'google' ? googleCount(connector.id) > 0 : Boolean(keys[CONNECTOR_KEYS[connector.id]]);

function keysPill() {
  const entries = Object.values(keys);
  if (!entries.length) return statePill('No keys yet');
  const healthy = entries.filter(({ status }) => status === 'healthy').length;
  return statePill(`${healthy}/${entries.length} healthy`, healthy === entries.length ? 'success' : 'warning', healthy === entries.length ? 'check' : '');
}

function keyRows() {
  const rows = CONNECTORS.filter(isConnected);
  if (!rows.length) return `<div class="empty-registry">${icon('plug')}<span>No keys added yet</span><button class="button secondary" data-action="browse">Add a key ${icon('arrow')}</button></div>`;
  return `<div class="key-list">${rows.map((connector) => {
    const provider = CONNECTOR_KEYS[connector.id];
    const entry = provider === 'google' ? keys.composio : keys[provider];
    const detail = provider === 'google' ? `${googleCount(connector.id)} active via Composio` : `Key ${maskKey(entry.apiKey)}${entry.account ? ` · ${entry.account}` : ''}`;
    const checked = timeLabel(entry.checkedAt);
    return `<article class="key-row"><span class="service-box">${mark(connector.icon)}</span><div class="key-meta"><strong>${esc(connector.name)}</strong><small>${esc(detail)}${checked ? ` · checked ${esc(checked)}` : ''}</small></div>${connectorPill(connector)}<button class="button secondary compact" data-action="key-connect" data-value="${connector.id}">Manage</button></article>`;
  }).join('')}</div>`;
}

function saveKeys(next) {
  keys = normalizeKeys(next);
  try { localStorage.setItem(KEYS_KEY, JSON.stringify(keys)); localStorage.removeItem(LEGACY_COMPOSIO_KEY); }
  catch { notify('Browser storage is unavailable. Keys will be forgotten when you leave.', 'info'); }
}

async function checkKey(provider, apiKey) {
  const state = modalState;
  if (!state || state.busy) return;
  state.busy = true; state.error = ''; renderDialog();
  const result = await requestKeyCheck(provider, apiKey);
  state.busy = false;
  if (result.status === 'error') { state.error = result.message; if (modalState === state) renderDialog(); return; }
  // A new key is only kept once the provider accepts it; an existing key keeps its latest status.
  if (result.status === 'healthy' || keys[provider]?.apiKey === apiKey) {
    const { status, account, checkedAt, toolkits } = result;
    saveKeys({ ...keys, [provider]: { apiKey, status, account, checkedAt, ...(toolkits ? { toolkits } : {}) } });
    state.draft = '';
  }
  if (result.status !== 'healthy') state.error = result.status === 'unauthorized' ? 'The provider rejected this key. Check it and try again.' : 'The provider could not be reached. Try Check again in a moment.';
  render(); if (modalState === state) renderDialog();
  if (result.status === 'healthy') notify(`${KEY_GUIDES[provider].name} connected.`);
}

function keyDialog() {
  const connector = CONNECTORS.find(({ id }) => id === modalState.id);
  const provider = CONNECTOR_KEYS[connector.id];
  const guide = KEY_GUIDES[provider];
  const steps = `<ol class="guide-steps">${guide.steps.map((step, index) => `<li><span>${index + 1}</span><div><p>${esc(step)}</p></div></li>`).join('')}</ol><p class="field-hint">${icon('info')}${esc(guide.note)}</p>`;
  const error = `<p class="form-error" role="alert">${esc(modalState.error || '')}</p>`;
  const link = `<a class="text-button" href="${guide.url}" target="_blank" rel="noopener noreferrer">Official page ${icon('external')}</a>`;
  if (provider === 'google') {
    const composio = keys.composio;
    const status = composio ? `<div class="key-row">${statePill(...KEY_STATE[composio.status])}<div class="key-meta"><strong>Composio</strong><small>${googleCount(connector.id)} active ${esc(connector.name)} account(s)${composio.checkedAt ? ` · checked ${esc(timeLabel(composio.checkedAt))}` : ''}</small></div></div>` : '';
    const action = composio ? `<button class="button primary" data-action="key-check" data-value="composio" ${modalState.busy ? 'disabled' : ''}>${modalState.busy ? 'Checking…' : 'Check Composio'}</button>` : '<button class="button primary" data-action="key-connect" data-value="composio">Add Composio key</button>';
    return `${heading(`Connect ${connector.name}`, 'Through your Composio project')}${steps}${status}${error}<div class="modal-footer">${link}${action}</div>`;
  }
  const entry = keys[provider];
  if (entry) {
    return `${heading(guide.name, guide.label)}${steps}<div class="key-row"><div class="key-meta"><strong>Key ${esc(maskKey(entry.apiKey))}</strong><small>${esc(entry.account || '')}${entry.checkedAt ? `${entry.account ? ' · ' : ''}checked ${esc(timeLabel(entry.checkedAt))}` : ''}</small></div>${statePill(...KEY_STATE[entry.status])}</div>${error}<div class="modal-footer">${link}<button class="button secondary" data-action="key-disconnect" data-value="${provider}" ${modalState.busy ? 'disabled' : ''}>Disconnect</button><button class="button primary" data-action="key-check" data-value="${provider}" ${modalState.busy ? 'disabled' : ''}>${modalState.busy ? 'Checking…' : 'Check'}</button></div>`;
  }
  return `${heading(guide.name, guide.label)}${steps}<form id="key-form" autocomplete="off"><div class="field"><label for="key-input">${esc(guide.label)}</label><input id="key-input" name="apiKey" type="password" autocomplete="off" spellcheck="false" maxlength="4096" placeholder="Paste your key" value="${esc(modalState.draft || '')}" required></div><p class="field-hint">${icon('lock')}Kept only in this browser. Sent to the server just to run the check, never stored there.</p>${error}<div class="modal-footer">${link}<button class="button primary" type="submit" ${modalState.busy ? 'disabled' : ''}>${modalState.busy ? 'Checking…' : `Connect ${icon('arrow')}`}</button></div></form>`;
}
...
function stackPage() {
  ...
  return `<section><div class="section-heading stack-heading"><div class="inline-group"><h2>My AI stack</h2><span class="small-label">Manual tracking</span>${help('stack')}</div><button class="button primary" data-action="add-stack">${icon('plus')}Add AI</button></div>
    ...
    ${summary.overlap.length ? `<div class="overlap-row"><span>Compare before renewing</span>${summary.overlap.map((item) => `<span class="tag">${esc(item.capability)} × ${item.count}</span>`).join('')}${help('overlap')}</div>` : ''}</section>`;
}
...
function connectorDialog() {
  const connector = CONNECTORS.find(({ id }) => id === modalState.id);
  return `${heading(connector.name)}<div class="connector-detail-logo">${mark(connector.icon)}${connectorPill(connector)}</div><div class="detail-block"><h3>Use it for</h3><p>${connector.benefit}</p></div><div class="detail-block"><h3>Access</h3><p>${connector.permission}</p></div><div class="modal-footer"><button class="button ghost" data-action="close">Close</button>${['claude', 'chatgpt'].includes(connector.id) ? `<button class="button secondary" data-action="import-from" data-value="${connector.name}">Import context</button>` : ''}<button class="button primary" data-action="key-connect" data-value="${connector.id}">${isConnected(connector) ? 'Manage key' : 'Connect'}</button></div>`;
}
...
const actions = {
  ...
  'confirm-clear': () => {
    try { localStorage.removeItem(STORAGE_KEY); localStorage.removeItem(STACK_KEY); localStorage.removeItem(KEYS_KEY); localStorage.removeItem(LEGACY_COMPOSIO_KEY); sessionStorage.removeItem(SESSION_KEY); sessionStorage.removeItem(CHAT_KEY); }
    catch { notify('Could not clear browser storage. Check browser permissions.', 'info'); return; }
    chatGeneration++; chatAbort?.abort(); chatAbort = null;
    profile = createProfile(); profileDraft = structuredClone(profile); sources = []; stack = []; chat = createChat(); composerDraft = ''; showSamples = false; keys = {};
...
function captureField(target) {
  ...
  if (target.id === 'key-input' && modalState?.kind === 'key') modalState.draft = target.value;
...
document.addEventListener('submit', async (event) => {
  ...
  if (!['profile-form', 'onboarding-profile-form', 'chat-form', 'note-form', 'stack-form', 'key-form'].includes(form.id)) return;
  event.preventDefault();
  ...
  if (form.id === 'key-form') {
    const key = String(modalState.draft || '').trim();
    if (!key) { modalState.error = 'Enter an API key.'; renderDialog(); return; }
    await checkKey(CONNECTOR_KEYS[modalState.id], key);
  }
