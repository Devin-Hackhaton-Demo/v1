import {
  ...
  COMPOSIO_KEY, normalizeComposio, maskKey, requestComposioCheck,
} from './state.mjs';
...
const HELP = {
  preview: ['About Coffeenator', 'Themes, profiles, text imports and manual subscription tracking work locally on this device. Chat replies are generated through the server-side Anthropic proxy. You can add your own Composio API key on the Connectors page; app authorization through Composio and cloud sync are not connected yet.'],
  ...
  composio: ['Composio API key', 'Add a project API key from your Composio dashboard. The key is kept only in this browser (localStorage). When you press Connect or Check, it is sent to this app’s server once to call the Composio API and is never stored or logged there. Healthy means Composio accepted the key at the time shown. Connecting individual apps through Composio is not available yet.'],
...
let composio = normalizeComposio(readStorage('localStorage', COMPOSIO_KEY));
let composioDraft = '';
let composioBusy = false;
let composioError = '';
let chatAbort = null;
...
function composioPill() {
  if (!composio.apiKey) return statePill('Not connected');
  return statePill(...(composio.status === 'healthy' ? ['Connected', 'success', 'check'] : COMPOSIO_STATE[composio.status]));
}

function composioSection() {
  const error = `<p class="form-error" role="alert">${esc(composioError)}</p>`;
  if (!composio.apiKey) {
    return `<section class="composio-section"><div class="section-heading"><h2>Composio API key</h2>${help('composio')}</div><form id="composio-form" class="composio-form" autocomplete="off"><div class="field"><label for="composio-key">API key</label><input id="composio-key" name="apiKey" type="password" autocomplete="off" spellcheck="false" maxlength="4096" placeholder="Paste your Composio API key" value="${esc(composioDraft)}" required></div><button class="button primary" type="submit" ${composioBusy ? 'disabled' : ''}>${composioBusy ? 'Checking…' : `Connect ${icon('arrow')}`}</button></form><p class="field-hint">${icon('lock')}Kept only in this browser. Sent to the server just to run the check, never stored there. <a class="text-button" href="https://platform.composio.dev" target="_blank" rel="noopener noreferrer">Get a key ${icon('external')}</a></p>${error}</section>`;
  }
  const checked = timeLabel(composio.checkedAt);
  return `<section class="composio-section"><div class="section-heading"><h2>Composio API key</h2>${help('composio')}</div><div class="composio-row"><span class="service-box">${icon('plug')}</span><div class="composio-meta"><strong>Composio</strong><small>Key ${esc(maskKey(composio.apiKey))}${composio.account ? ` · ${esc(composio.account)}` : ''}${checked ? ` · checked ${esc(checked)}` : ''}</small></div>${statePill(...COMPOSIO_STATE[composio.status])}<div class="composio-actions"><button class="button secondary compact" data-action="composio-check" ${composioBusy ? 'disabled' : ''}>${composioBusy ? 'Checking…' : 'Check'}</button><button class="button secondary compact" data-action="composio-disconnect" ${composioBusy ? 'disabled' : ''}>Disconnect</button></div></div>${error}</section>`;
}

function saveComposio(next) {
  composio = normalizeComposio(next);
  try { if (composio.apiKey) localStorage.setItem(COMPOSIO_KEY, JSON.stringify(composio)); else localStorage.removeItem(COMPOSIO_KEY); }
  catch { notify('Browser storage is unavailable. The key will be forgotten when you leave.', 'info'); }
}

async function checkComposio(apiKey) {
  if (composioBusy) return;
  composioBusy = true; composioError = ''; render();
  const result = await requestComposioCheck(apiKey);
  composioBusy = false;
  if (result.status === 'error') { composioError = result.message; render(); return; }
  // A new key is only kept once Composio accepts it; an existing key keeps its latest status.
  if (result.status === 'healthy' || composio.apiKey === apiKey) {
    saveComposio({ apiKey, status: result.status, account: result.account, checkedAt: result.checkedAt });
    composioDraft = '';
  }
  if (result.status !== 'healthy') composioError = result.status === 'unauthorized' ? 'Composio rejected this key. Check it and try again.' : 'Composio could not be reached. Try Check again in a moment.';
  render();
  if (result.status === 'healthy') notify('Composio key connected.');
}

function connectorsPage() {
  return `<div class="page"><div class="page-heading"><h1>Connectors</h1><div class="provider-label"><span>Composio</span>${composioPill()}${help('composio')}</div></div><div class="section-tabs" role="group" aria-label="Connector view"><button class="section-tab${selected(connectorTab === 'connections')}" data-action="connector-tab" data-value="connections" aria-pressed="${connectorTab === 'connections'}">${icon('plug')}Connections</button><button class="section-tab${selected(connectorTab === 'stack')}" data-action="connector-tab" data-value="stack" aria-pressed="${connectorTab === 'stack'}">${icon('cpu')}My AI stack</button></div>${connectorTab === 'stack' ? stackPage() : composioSection() + registryPage()}</div>`;
}
...
const actions = {
  ...
  'composio-check': () => checkComposio(composio.apiKey),
  'composio-disconnect': () => { saveComposio(null); composioError = ''; render(); notify('Composio key removed from this browser.'); },
  ...
  'confirm-clear': () => {
    try { localStorage.removeItem(STORAGE_KEY); localStorage.removeItem(STACK_KEY); localStorage.removeItem(COMPOSIO_KEY); sessionStorage.removeItem(SESSION_KEY); sessionStorage.removeItem(CHAT_KEY); }
    catch { notify('Could not clear browser storage. Check browser permissions.', 'info'); return; }
    chatGeneration++; chatAbort?.abort(); chatAbort = null;
    profile = createProfile(); profileDraft = structuredClone(profile); sources = []; stack = []; chat = createChat(); composerDraft = ''; showSamples = false; composio = normalizeComposio(null); composioDraft = ''; composioError = '';
...
function captureField(target) {
  ...
  if (target.id === 'composio-key') composioDraft = target.value;
...
document.addEventListener('submit', async (event) => {
  ...
  if (!['profile-form', 'onboarding-profile-form', 'chat-form', 'note-form', 'stack-form', 'composio-form'].includes(form.id)) return;
  event.preventDefault();
  ...
  if (form.id === 'composio-form') {
    const key = composioDraft.trim();
    if (!key) { composioError = 'Enter a Composio API key.'; render(); return; }
    await checkComposio(key);
  }
