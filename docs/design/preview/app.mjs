...
const HELP = {
  preview: ['About this preview', 'Themes, profiles, text imports and manual subscription tracking work locally. Chat replies are generated through this preview server’s Anthropic proxy. Google sign-in, Composio authorization and cloud sync are not connected yet.'],
  ai: ['AI connection', 'Chat replies are generated by Anthropic through this preview server. Your conversation is sent to the backend only when you press Send, and the API key stays on the server — it is never in the browser. Never paste API keys into a chat, import, or browser setting.'],
  privacy: ['Local data', 'Chat messages are sent to this preview server and forwarded to Anthropic only when you press Send; the API key stays on the server. Preferences and AI tracking stay on this device. Imported text stays in this tab’s session and is not uploaded: export a copy before closing, because it is not synced or backed up. This is not a production security guarantee.'],
...
function chatPage() {
  ...
  return `<div class="page chat-page${chat.turns.length || busy ? ' has-messages' : ''}${brewing ? ' is-brewing' : ''}"><div class="chat-tools"><button class="text-button" data-action="new-chat">${icon('plus')}New chat</button><div class="chat-tools-right">${onboardingProgress(profile).completed < 3 ? '<button class="text-button" data-action="onboarding">Personalize</button>' : ''}<button class="icon-button" data-action="animation" aria-label="${brewing ? 'Stop animation preview' : 'Preview brewing animation'}" title="Animation preview — not an AI request">${icon(brewing ? 'stop' : 'play')}</button></div></div>
      ...
      <div class="composer-window${brewing || busy ? ' brewing' : ''}"><div class="composer-aura"></div><form id="chat-form" class="composer"><label class="sr-only" for="chat-input">Your message</label><textarea id="chat-input" rows="3" maxlength="2000" placeholder="Ask a question or give me a task…">${esc(composerDraft)}</textarea><div class="composer-bottom"><div class="composer-actions"><button type="button" class="icon-button" data-action="import" aria-label="Attach context" title="Attach context">${icon('plus')}</button><a class="context-chip" href="#memory">${icon('memory')}<span>Memory</span><strong>${sources.length}</strong></a></div><button class="button send-button" type="submit" ${busy ? 'disabled' : ''}>${busy ? 'Sending…' : `Send ${icon('arrow')}`}</button></div></form></div>
      <div class="composer-caption"><span>${brewing ? '<span class="brewing-dot"></span>Animation preview · no AI request' : busy ? '<span class="brewing-dot"></span>Sending to the server…' : 'Replies via the server-side Anthropic proxy'}</span>${help('ai')}</div>
