...
- Composio: the user pastes their own Composio API key on the Connectors page. It is stored only in the browser's localStorage (`coffeenator-preview-composio-v1`) and sent to the stateless `POST /api/composio/check` (body `{ apiKey }`), which calls Composio `GET /api/v3.1/connected_accounts?limit=1` and returns only `{ healthy, account | reason }`. The key is never stored, logged or echoed server-side. A new key is kept only after Composio accepts it. App authorization through Composio is still not implemented.
