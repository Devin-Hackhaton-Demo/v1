...
import { CHECK_RATE_LIMIT, handleKeyCheck } from '../_lib/connections.mjs';
...
export default async function handler(request, response) {
  ...
  if (!guardPost(request, response, 'Use POST to check an API key.')) return;
  if (!guardRateLimit(request, response, rateLimiter)) return;
  if (!guardBodySize(request, response)) return;
  const { status, payload } = await handleKeyCheck(parsedBody(request));
